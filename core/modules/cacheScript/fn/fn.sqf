...
if(_mkr=="mkr")then{_mkrs=_mkrs+[_x];};}forEach _mkrsTotal;
_mkrs;};
SL_findHouses={
...
_cbPos=0;
for "_x" from 1 to 100 do{if(format["%1",(_house buildingPos _x)]!="[0,0,0]")then{_cbPos=_cbPos+1;};};
_cbPos;};
SL_randHousePos={
private["_house","_count","_pos"];
_house=_this select 0;
_count=[_house]call SL_housePos;
if(_count==0)then{_pos=getPos _house;}else{_pos=random _count;_pos=_house buildingPos _pos;};
if((_pos select 0)==0)then{_pos=getPos _house;};
_pos};