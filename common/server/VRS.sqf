...
sleep 1;};
	if((_abandoned)&&{_veh distance _pos>10})then{deleteVehicle _veh;
sleep 1;
	_veh=createVehicle[_vehType,_pos,[],0,"CAN_COLLIDE"];_veh allowDamage false;_veh setDamage 0;
	...
	if(_veh isKindOf Tooth_OPFOR_CAS_drone)then{_veh enableUAVConnectability[_veh,true];_veh enableUAVWaypoints true;};
...
missionNamespace setVariable[_vehName,_veh];publicVariable _vehName;
_veh call _vehInit;sleep 3;_veh allowDamage true;_veh setDamage 0;};
};
...
sleep 1;};
	if(_dead)then{deleteVehicle _veh;
sleep 1;
	_veh=createVehicle[_vehType,_pos,[],0,"CAN_COLLIDE"];_veh allowDamage false;_veh setDamage 0;
	_veh setDir _dir;_veh setPos[_pos select 0,_pos select 1,0];
	clearItemCargoGlobal _veh;clearBackpackCargoGlobal _veh;_veh setDamage 0;_veh lock 0;
