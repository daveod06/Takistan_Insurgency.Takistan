...
hint 'Selected munition not available!';
playSound 'cantDo';};
if((whatIsChecked==2)&&(_x=='GUNS'))then{
_display=uiNamespace getVariable 'casMenu';
_button=_display displayCtrl 100112;
_button ctrlEnable false;
hint 'Selected munition not available!';
...
_button=_display displayCtrl 100112;
_button ctrlEnable true;};
if(isClicked &&(whatIsChecked==2)&&(_x != 'GUNS'))then{
_display=uiNamespace getVariable 'casMenu';
_button=_display displayCtrl 100112;
_button ctrlEnable true;};
...
if(doSnap)then{_toggle ctrlSetText "<Enabled>";_toggle ctrlSetTextColor[0,1,0,1];}else{_toggle ctrlSetText "<Disabled>";_toggle ctrlSetTextColor[1,0,0,1];};
if(abortCAS)then{_button=_display displayCtrl 100113;_button ctrlEnable false;}else{_button=_display displayCtrl 100113;_button ctrlEnable true;};
...
if((whatIsChecked==2)&&(_x=='GUNS'))exitWith{
_button ctrlEnable false;
hint 'Selected munition not available!';
playSound 'cantDo';
_exit=true;};
...
_dist=999;
{if((_x distance _pos<_dist)&&(player knowsAbout _x>2.5))then{nearestVeh=_x;};}forEach nearTargetVehList;}else{
...
if((doSnap)&& !isNull nearestVeh)then{"CAS_TARGET" setMarkerPos(getPos nearestVeh);};
if((player distance targetPos>=3)&& !(doSnap))then{"CAS_TARGET" setMarkerPos(targetPos);};
sleep 0.3;};
...
if !(casRequest)then{deleteMarker "CAS_TARGET";}else{[_owner,maxDisReq,_lockToOwner,_num,casType,"CAS_TARGET",_id]execVM "common\client\CAS\CAS.sqf";};