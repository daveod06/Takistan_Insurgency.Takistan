...
//IF IS ADDACTION
if(count _this>=3)then{_isAction=(_this select 3);
	if(typeName _isAction=="ARRAY")then{_addAction=true;_BRSarray=(_this select 3);}else{_addAction=false;_BRSarray=_this;};}else{_addAction=false;_BRSarray=_this;};
if(count _BRSarray==0)then{_customRP=[];}else{
	if(count(_BRSarray select 0)==0)then{_customRP=[];}else{_customRP=(_BRSarray select 0);};};
