...
case 3:{if(r_vCH isEqualTo[FALSE,FALSE])then{
		...
		if(side player==SIDE_OCCUPIERS)then{if(channelEnabled ch6 isEqualTo[False,False])then{ch6 radioChannelAdd[player];};(ch6+5)enableChannel r_sCH;setCurrentChannel(ch6+5);};
		if(side player==SIDE_INS)then{if(channelEnabled ch7 isEqualTo[False,False])then{ch7 radioChannelAdd[player];};(ch7+5)enableChannel r_sCH;setCurrentChannel(ch7+5);};
		if(side player==SIDE_IND)then{if(channelEnabled ch8 isEqualTo[False,False])then{ch8 radioChannelAdd[player];};(ch8+5)enableChannel r_sCH;setCurrentChannel(ch8+5);};
...
case 4:{if((r_sCH isEqualTo[FALSE,FALSE])&&(leader(group(vehicle player))==player))then{2 enableChannel r_cCH;setCurrentChannel 2;}else{
		...
		if(side player==SIDE_OCCUPIERS)then{if(channelEnabled ch6 isEqualTo[False,False])then{ch6 radioChannelAdd[player];};(ch6+5)enableChannel r_sCH;setCurrentChannel(ch6+5);};
		if(side player==SIDE_INS)then{if(channelEnabled ch7 isEqualTo[False,False])then{ch7 radioChannelAdd[player];};(ch7+5)enableChannel r_sCH;setCurrentChannel(ch7+5);};
		if(side player==SIDE_IND)then{if(channelEnabled ch8 isEqualTo[False,False])then{ch8 radioChannelAdd[player];};(ch8+5)enableChannel r_sCH;setCurrentChannel(ch8+5);};}else{
